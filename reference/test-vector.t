...
foreach my $test ( @tests ) {
    ...
    my  $newcipher = qx(./xxtea encode $key $plaintext );
    chomp $newcipher;

    my  $newplaintext = qx( ./xxtea decode $key $ciphertext );
    chomp $newplaintext;

    is( $newcipher, $ciphertext, "$blockType$blockTest (encoding)" )
        or diag( "./xxtea encode $key $plaintext" );
    is( $newplaintext, $plaintext, "$blockType$blockTest (decoding)" )
        or diag( "./xxtea decode $key $ciphertext" );
    $blockTest++;
...
# ./xxtea encode 00000000000000000000000000000000 0000000000000000
__DATA__
# 64-bit block
